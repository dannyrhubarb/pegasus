...
- **UI**: a **VS button in the replay bar's left `.rp-side`** (until now an
  ...
  transport centred) opening `#rp-ghost-menu`, the speed picker's twin on
  the left in magenta. Candidates are the **cached board the replay was
  launched from** (`armCompare`, called at both replay entry points) minus
  the run being watched — so opening the picker costs no fetch; the blob is
  pulled only on pick, like ▶ watch. The button hides when there is nothing
  to race. Pilot names go in via `textContent`, never `innerHTML`.
