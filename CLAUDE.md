...
- `robots.txt` — the **crawl policy** (2026-09, after Search Console
  ...
  link `pr-<n>/`). **The game page itself is `noindex`** (2026-09-25,
  owner decision): `index.html` carries `<meta name="robots"
  content="noindex">` because Google was listing `/play/` NEXT TO the
  landing page, with a snippet scraped from the dormant scr-announce
  screen's hidden "low-effort terror" text — the game is an app with
  nothing to read, and the site root (pitch, store badges, the browser
  build one tap away) is the one result we want. A meta tag, not a
  robots.txt rule (a disallowed URL can still be indexed from links, and
  a blocked crawl never sees the tag); harmless in the shells and the
  PWA, and since the previews bundle the same page it covers them too,
  so the `-pr-<n>`-stamped variant once planned here is moot.
  `editor.html` carries the same tag. `index.html` also gained a meta
  description + the landing page's Open Graph set (`og:url` = `/play/`
  itself: sharing the browser build means the browser build) — link
  previews read those whether or not a page is indexed, and a shared
  `/play/` link used to preview the announcement text
