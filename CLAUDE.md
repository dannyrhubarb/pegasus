...
- **No build time any more (2026-09)**: the About screen's "Build time"
  row, the `__BUILD_TIME__` stamp and the script's `--commit-date` mode
  are gone. The row had become the commit date (a wall clock was the one
  input that could never reproduce byte-for-byte, #214), which the
  Version + revision rows already pin — and `index.html` stays a pure
  function of the commit without it.
...
- **`ci.yml`'s `reproducible` job is the proof, on every PR**: two
  ...
  unpinned tool or a path leaking into the binary. (The old build-time
  stamp was retired for this reason — see "Versioning".)
...
- **scr-about**: **Version** + **Source revision** (the commit sha — labelled
  "source" not "build" since 2026-09: in the shells it sits under a
  Version row that IS the store build; deploy-time `sed` of
  the `__BUILD_VERSION__` / `__GIT_REVISION__` placeholders by
  `build-site`; local fallback "dev (local build)" via
  `startsWith("__")`). On the website Version is the tag-derived
  `1.3.0+14` (see "Versioning"); **in the app shells the SAME row shows
  the INSTALLED app instead** — "1.3.0 (42)", marketing version + the CI
  run-number build, read from the shell bridges (Android
  `PegasusApp.appBuild()`, iOS the `window.__pegAppBuild` document-start
  user script) — one version per platform (owner decision 2026-09; the
  separate App build row and the Build time row were dropped with it).
  The baked tag-derived stamp still lives in `rawBuildVersion` for the
  update policy's `minVersion` and the bug-report header, which logs
  both. The revision carries NO platform suffix any more (the old `-ios`
  / `-android` went the same day — the Version row and the analytics
  device-mix enums already say which shell a build runs in; the replay
  build id only ever read the hex prefix; the PR test APK keeps its
  `-pr-<n>` marker). Then the
...
- `minBuild` `{android, ios}`: a shell build (CI run number, the "(42)"
  in the About screen's Version row) below the platform's number gets the
...
- **World-to-screen**: a per-frame closure `w2s` (defined inside the `loop {}`, AFTER the frame's ship pose and camera look-ahead are known — it captures both) converts world coords to screen pixels using `view_scale`. **The `(ax, ay)` anchor every call passes is the SHIP's rendered pose (`cam_x`/`cam_y` — that pair IS the ship, not the camera, despite the name); the camera centre is the anchor plus `cam_look`.**
- **Camera look-ahead (2026-09)**: the view LEADS the ship in its direction of travel — what's behind you is of little use, the rock ahead is what matters. `cam_look` (world m, frame-loop state) eases toward `look_ahead_target(vx, vy, half_w, half_h)` (pure, unit-tested): `LOOK_AHEAD_SECS = 0.9` s of travel per axis, soft-saturated (tanh) at `LOOK_AHEAD_FRAC = 0.35` of the HALF-viewport per axis — so at full lead the ship sits ~⅓ in from its trailing edge with two thirds of the screen ahead, never leaves the middle third however fast it falls down a shaft, and a hover is centred. `ease_look_ahead` is an exponential approach with `LOOK_AHEAD_TAU = 0.5` s (frame dt clamped, so a hitch can't overshoot). It reads the RENDERED velocity — the replayed ship's in playback, so a watched run shows the view its pilot saw — and is **FROZEN once the rendered ship is a wreck** (`sim.crashed` / the scratch sim's): the parked wreck's velocity is 0 and easing back would pan the camera during the explosion, which the wreck-park exists to prevent. Reset and level-load zero it with the other per-run cosmetics (the fresh ship stands still on the spawn; easing back from the old lead would pan onto it instead of cutting). Pure presentation: the sim, recorder, ghost and verifier never see the camera. **Everything world-space goes through `w2s`, so it follows for free; the four things that DON'T are handled explicitly** — the radial-light `ship_pos` uniform and the velocity-vector arrow origin use `w2s(cam_x, cam_y, …)` (the ship's actual, off-centre screen position — never `sw/2, sh/2` any more), the hand-drawn terrain's world-space cull rect is centred on `cam ± look`, the star parallax follows the camera centre, and the minimap's viewport rectangle leads the (still centred) ship dot by the lead. The minimap itself and every collider/render WINDOW stay keyed on the ship — the lead is ≤ ~7 m, far inside `HALF_WINDOW`. Add any new "the ship is at the screen centre" assumption via `w2s`, not `sw/2`.
...
scale on the 480×160 map; the ship dot always sits at the centre and the viewport rect leads it by the camera look-ahead — see "Rendering architecture").
...
- `ios/sync-web.sh` assembles `ios/Pegasus/WebRoot/` (**gitignored** build
  ...
  scores off), and the injected revision is the plain sha (the `-ios`
  suffix was dropped 2026-09 — the page env-tags these builds `prod` and
  the device-mix enums show them as iOS webview, which is all the suffix
  ever added); the `__BUILD_VERSION__` stamp comes from `tools/version.sh`
  like the deploy's (see "Versioning").
...
- `android/sync-web.sh` assembles `app/src/main/assets/webroot/`
  ...
  and `tools/build-site.sh` (keep all three in sync); plain-sha
  revision (no `-android` suffix since 2026-09), no `version.json`, `config.json` from
...
- **On-demand PR test APK** (`android-test-apk.yml`, 2026-07): put a
  **`test-apk` label** on a PR and it builds one, refreshing on every push
  ...
  `PEGASUS_VERSION_SUFFIX` so the About screen's Version row reads
