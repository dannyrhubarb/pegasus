...
- **`view_scale`**: `SCALE * 0.38 * dpi` on small screens (`sw.min(sh)/dpi < 600` CSS px, i.e. mobile in either orientation), `SCALE * dpi` on desktop. Controls zoom; HUD/minimap are unaffected. Keyed on the *smaller* dimension (not `sw`) so a phone keeps the same zoom across portrait/landscape — using `sw` alone made landscape flip to desktop zoom and appear zoomed-in.
