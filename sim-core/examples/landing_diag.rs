...
use pegasus_sim::sim::{Sim, PHYSICS_DT};
use pegasus_sim::world::{Level, PAD_HALF_W, PAD_LAND_TIME};
...
fn main() {
    ...
    for tick in k0.tick..rec.ticks() {
        ...
        let feet = y - 0.73;
        let timer = sim.land_progress() * PAD_LAND_TIME;
        ...
        if (interesting || was_near) && let Some((key, cx, dx, dy)) = near {
            ...
            let fails: Vec<&str> = [
                (dx > PAD_HALF_W).then_some("off-deck-x"),
                (dy >= 0.3).then_some("feet-not-on-deck"),
            ...
            if timer > 0.7 * PAD_LAND_TIME || tick % 6 == 0 || rep.impact.is_some() || rep.landed {
                println!(
